...
export default function AddPageNumbers() {
  ...
  const addNumbers = async () => {
    ...
    } catch(e) { alert("Error adding page numbers."); }
    setProcessing(false);
  };
    ...
    >
      {!file ? <UploadZone onFiles={loadFile} label="Drop your PDF here" /> : (
        <div>
          ...
          <div style={{ background: "#f9fafb", border: "1px solid #f3f4f6", borderRadius: 14, padding: 20, marginBottom: 20, display: "flex", flexDirection: "column", gap: 20 }}>
            <div>
              <p style={{ fontWeight: 600, color: "#374151", fontSize: 14, marginBottom: 12 }}>Position</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {positions.map(p => (
                  <button key={p.val} onClick={() => setPosition(p.val)} style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 8 }}>Start from page</label>
                <input type="number" min="1" value={startNum} onChange={e => setStartNum(parseInt(e.target.value) || 1)}
                  style={{ width: "100%", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "10px 14px", fontSize: 14, fontFamily: "inherit", outline: "none" }} />
              </div>
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 8 }}>Font size</label>
                <select value={fontSize} onChange={e => setFontSize(parseInt(e.target.value))}
                  style={{ width: "100%", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "10px 14px", fontSize: 14, fontFamily: "inherit", outline: "none", background: "white" }}>
          ...
          <button onClick={addNumbers} disabled={processing} style={{ width: "100%", background: processing ? "#d1d5db" : "#E8380D", color: "white", border: "none", padding: 16, borderRadius: 14, fontWeight: 700, fontSize: 16, cursor: processing ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
      ...
      {downloadUrl && (
        <div style={{ marginTop: 24, background: "#F0FDF4", border: "1px solid #bbf7d0", borderRadius: 16, padding: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
            ...
            <div>
              <p style={{ fontWeight: 700, color: "#111827", fontSize: 17 }}>Page numbers added!</p>
              <p style={{ fontSize: 13, color: "#6b7280" }}>Starting from {startNum}, positioned {position.replace("-", " ")}.</p>
            </div>
          </div>
